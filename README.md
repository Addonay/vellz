...
```sh
zig build test     # 845/845 unit and integration tests (837 lib + 8 scene)
zig build corpus   # 95/95 corpus scenes byte-exact vs the upstream oracle
...
the pinned oracle, atlas cache on and off (90/90 corpus, tolerance 0). T5 adds embedded
bitmap glyphs (`CBDT`/`CBLC`, `sbix`, `EBDT`/`EBLC`), the upstream
COLR > bitmap > outline cascade, the pending-upload atlas path, and a
`png 0.18`-compatible PNG decoder (indexed `PLTE`+`tRNS`, filters 0–4); its
5-scene G3d corpus (Noto CBTF colour emoji, cache on/off, stroked and
transform-composition rows) renders byte-exact at tolerance 0, so the corpus is
95/95. The GPU
...
outlines (the interpreter), glyph decoration and the Cozmic
