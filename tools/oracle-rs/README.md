...
| `hint` | default `true` (upstream); the ported `Engine::AutoFallback` (interpreter or autohinter) runs hinted runs |
...
| `embolden` | optional `[x, y]`; synthetic dilation via `kurbo::expand_path` (positive amounts), applied after hinting |
...
| `decoration` | optional skip-ink decoration drawn after the fill/stroke pass (T5), with the run's embolden settings |
