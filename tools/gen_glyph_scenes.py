...
def dump_advances_at(glyph_ids, font, size):
    ...
    for line in text.splitlines():
        ...
        if len(parts) == 6 and parts[0] == "gid" and parts[2] == "lsb":
            # `format_opt_f32` prints the f32 bit pattern as a hexadecimal
            # word in natural (big-endian) order; decoding it little-endian
            # yields ~1e-38 denormals and collapses every run to x = 0.
