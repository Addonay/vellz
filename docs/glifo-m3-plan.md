...
M3 remainder after these five: autohint, `gvar`/variation, CFF — each explicit `error.Unsupported`. (TrueType hinting, bitmap/PNG, decoration, the Cozmic adapter, synthetic embolden and PNG 16-bit/Adam7 landed on later M3 branches; see `plan.md`'s ledger.)
...
- Deferrals (CFF, variation, autohint) shrink coverage vs upstream test names; synthetic embolden and PNG 16-bit/Adam7 are ported (the latter has unit tests only, because the pinned fonts carry no such payloads).
