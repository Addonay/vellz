...
pub const MultiThreadedDispatcher = struct {
    ...
    ///
    /// `mask` ownership transfers on success; on failure it is released here
    /// (the caller must not release it again), matching the single-threaded
    ...
    fn rasterizeF32(
        ...
        var filters = FilterContext.init(0);
