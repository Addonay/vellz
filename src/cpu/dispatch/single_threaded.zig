...
pub const SingleThreadedDispatcher = struct {
    ...
    fn rasterizeWith(
        ...
        var filter_ctx = try self.rasterizeFilterLayers(
            K,
            allocator,
            encoded_paints,
            image_resolver,
    ...
    fn rasterizeFilterLayers(
        ...
        encoded_paints: []encode_mod.EncodedPaint,
        image_resolver: paint_mod.ImageResolver,
        timings: ?*settings_mod.StageTimings,
    ) !FilterContext {
    ...
    fn bucketAndRasterize(
        ...
        const bucket_start = if (timings) |t| t.now_ns() else 0;
        ...
        if (timings) |t| t.bucket_ns += t.now_ns() - bucket_start;
        ...
        const fine_start = if (timings) |t| t.now_ns() else 0;
        regions.update(&ctx);
        if (timings) |t| t.fine_ns += t.now_ns() - fine_start;
