...
//! `blurred_rect.zig`, and dispatches painter selection on `K.Numeric`: the u8
//! kernel uses the u8-native bilinear image painters and the u8 LUT gradient
//! painter where upstream's `U8Kernel` overrides the defaults; the remaining
//! painters stay f32 and expose `paintU8` for the conversion. Still deferred
//! (explicit `error.Unsupported`, never placeholder pixels): external
//! textures.
//!
