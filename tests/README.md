...
All scenes above render byte-exact with `zig build corpus` (95/95,
`tolerance=0`: 32 quality/f32 + 16 speed/u8 + 15 outline glyph + 27 COLR + 5
bitmap). The f32 and u8 pipelines are not byte-equal to each other in general
(integer `div_255` rounding vs f32); the corpus compares each pipeline against
its own oracle output, never against the other pipeline.
...
Planned: hinted glyph outlines (G3b), glyph decoration (T5), mixed scripts,
resource exhaustion, repeated resize.
